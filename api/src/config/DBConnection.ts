...
export default class DBConnection {
  ...
  constructor(private config: PoolConfig, private logger: Logger) {
  ...
  private makeNewConnection = async () => {
  ...
  private retry = () => {
    ...
    const interval = Math.min(
      2 * 60 * 1000,
      this.retrycount * this.CONNECTION_RETRY,
    );
    this.logger.error(
      `DBConnection: Attemping reconnection in ${interval /
        1000} seconds... - ${this.retrycount} attempt`,
    );
